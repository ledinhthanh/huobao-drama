  ...
  <div class="page" v-if="drama">
        ...
        <div class="head-info">
          ...
          <div class="page-meta">
            ...
            <span class="meta-item">
              ...
              {{ t('pages.drama.characterCount', { n: drama.characters?.length || 0 }) }}
            </span>
            ...
            <span class="meta-item">
              ...
              {{ t('pages.drama.sceneCount', { n: drama.scenes?.length || 0 }) }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="ep-grid">
      ...
      >
        ...
        <div class="ep-body">
          ...
          <div class="ep-status">
            ...
            <span class="status-text">{{ hasScript(ep) ? t('pages.drama.scriptReady') : t('pages.drama.noScript') }}</span>
      ...
      <div v-if="!drama.episodes?.length" class="card ep-empty">
        ...
        <p>{{ t('pages.drama.emptyEpisodes') }}</p>
      </div>
    </div>
        ...
        <div class="dialog-head">
          <div class="dialog-head-copy">
            ...
            <div class="dialog-title-row">
              <div class="dialog-title">{{ t('pages.drama.createEpisode') }}</div>
              <span class="dialog-badge">{{ t('pages.drama.dialogBadge') }}</span>
            </div>
            <div class="dialog-sub">{{ t('pages.drama.dialogSub') }}</div>
          </div>
          <button class="back-btn" @click="addDialog = false">{{ t('common.cancel') }}</button>
        </div>
        <div class="dialog-summary">
          <div class="summary-chip">{{ t('pages.drama.summaryImage', { n: imageConfigs.length }) }}</div>
          <div class="summary-chip">{{ t('pages.drama.summaryVideo', { n: videoConfigs.length }) }}</div>
          <div class="summary-chip">{{ t('pages.drama.summaryAudio', { n: audioConfigs.length }) }}</div>
        </div>
        <div class="dialog-body">
          <div class="dialog-section">
            <div class="dialog-section-head">
              <span class="dialog-section-title">{{ t('pages.drama.sectionBaseInfo') }}</span>
              <span class="dialog-section-copy">{{ t('pages.drama.sectionBaseInfoCopy') }}</span>
            </div>
            <label class="field">
              <span class="field-label">{{ t('pages.drama.fieldTitle') }}</span>
              <input v-model="newEpisodeTitle" class="input" :placeholder="t('pages.drama.fieldTitlePlaceholder')" />
              <span class="field-hint">{{ t('pages.drama.fieldTitleHint') }}</span>
            </label>
          </div>

          <div class="dialog-section">
            <div class="dialog-section-head">
              <span class="dialog-section-title">{{ t('pages.drama.sectionGenConfig') }}</span>
              <span class="dialog-section-copy">{{ t('pages.drama.sectionGenConfigCopy') }}</span>
            </div>
            <div class="config-grid">
              <label class="config-card">
                <span class="config-card-kicker">{{ t('pages.drama.imageKicker') }}</span>
                <span class="field-label">{{ t('pages.drama.fieldImageConfig') }}</span>
                <BaseSelect v-model="newEpisodeImageConfigId" :options="imageConfigOptions" :placeholder="t('pages.drama.imageConfigPlaceholder')" searchable />
              </label>
              <label class="config-card">
                <span class="config-card-kicker">{{ t('pages.drama.videoKicker') }}</span>
                <span class="field-label">{{ t('pages.drama.fieldVideoConfig') }}</span>
                <BaseSelect v-model="newEpisodeVideoConfigId" :options="videoConfigOptions" :placeholder="t('pages.drama.videoConfigPlaceholder')" searchable />
              </label>
              <label class="config-card">
                <span class="config-card-kicker">{{ t('pages.drama.audioKicker') }}</span>
                <span class="field-label">{{ t('pages.drama.fieldAudioConfig') }}</span>
                <BaseSelect v-model="newEpisodeAudioConfigId" :options="audioConfigOptions" :placeholder="t('pages.drama.audioConfigPlaceholder')" searchable />
              </label>
            </div>
          </div>
        </div>
        <div class="dialog-foot">
          <div class="dialog-foot-copy">{{ t('pages.drama.configLockedNote') }}</div>
          <button class="btn btn-primary" :disabled="creatingEpisode || !canCreateEpisode" @click="addEpisode">
            {{ creatingEpisode ? t('pages.drama.creating') : t('pages.drama.createAndLock') }}
          </button>
        </div>
      </div>
    </div>
