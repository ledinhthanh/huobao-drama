  ...
  <div class="settings-layout">
    ...
    <div class="settings-content">
      ...
      <div v-if="tab === 'ai'" class="settings-scroll">
          ...
          <div class="settings-brand">
            <div class="settings-brand-mark">
              <img
                v-if="showBrandImage"
                :src="brandLogo"
                :alt="t('pages.settings.brand.alt')"
                class="settings-brand-logo"
                @error="showBrandImage = false"
              />
              <span v-else class="settings-brand-fallback">{{
                t("layouts.default.brandMark")
              }}</span>
            </div>
            <div class="settings-brand-copy">
              <div class="settings-brand-kicker">
                {{ t("pages.settings.brand.kicker") }}
              </div>
              <div class="settings-brand-name">
                {{ t("pages.settings.brand.name") }}
              </div>
            </div>
          </div>
          <h2 class="settings-title">
            {{ t("pages.settings.aiServices.title") }}
          </h2>
          ...
          <div class="setup-panel-head">
            <div>
              <div class="setup-kicker">
                {{ t("pages.settings.aiServices.presetKicker") }}
              </div>
              <div class="setup-title">
                {{ t("pages.settings.aiServices.presetTitle") }}
              </div>
              <div class="setup-desc">
                {{ t("pages.settings.aiServices.presetDesc") }}
              </div>
            </div>
            <button class="btn btn-primary" @click="presetDialog = true">
              <Sparkles :size="14" />
              {{ t("pages.settings.aiServices.presetButton") }}
            </button>
          </div>
        ...
        <section class="setup-panel card">
          <div class="setup-panel-head compact">
            <div>
              <div class="setup-title">
                {{ t("pages.settings.aiServices.quickTemplate") }}
              </div>
              <div class="setup-desc">
                {{ t("pages.settings.aiServices.quickTemplateDesc") }}
              </div>
            </div>
          </div>
          ...
          <section v-for="st in serviceTypes" :key="st.type">
            <div class="section-head">
              <div>
                ...
                <div class="section-subtitle">
                  {{ t(`pages.settings.aiServices.serviceDesc.${st.type}`) }}
                </div>
              </div>
              <span v-if="countActive(st.type)" class="tag tag-accent">{{
                t("pages.settings.aiServices.enabledCount", {
                  n: countActive(st.type),
                })
              }}</span>
              <button
                ...
                @click="startAddCfg(st.type)"
              >
                <Plus :size="13" /> {{ t("pages.settings.aiServices.add") }}
              </button>
            </div>
            <div class="config-list">
              ...
              >
                <div class="config-info">
                  <div class="config-main">
                    ...
                    <span class="config-base mono truncate">{{
                      c.base_url || t("pages.settings.aiServices.notSetBaseUrl")
                    }}</span>
                  </div>
                </div>
                <span
                  ...
                  >{{
                    c.api_key
                      ? t("pages.settings.aiServices.configured")
                      : t("pages.settings.aiServices.noKey")
                  }}</span
                >
                <button
                ...
                >
                  {{ t("pages.settings.aiServices.test") }}
                </button>
                <label class="toggle"
                  ><input
                    type="checkbox"
              ...
              <p v-if="!byType(st.type).length" class="config-empty">
                {{ t("pages.settings.aiServices.empty") }}
              </p>
            </div>
          </section>
        </div>
          ...
          <div v-for="a in agentDefs" :key="a.type" class="card agent-card">
            <div class="agent-card-head" @click="toggleAgentEdit(a.type)">
              ...
              <span v-if="getAgentCfg(a.type)" class="tag tag-success">{{
                t("pages.settings.agents.configured")
              }}</span>
              <span v-else class="tag">{{
                t("pages.settings.agents.default")
              }}</span>
              <ChevronDown
                :size="14"
                :style="{
            ...
            <div v-if="editingAgent === a.type" class="agent-card-body">
              <label class="field">
                <span class="field-label"
                  >{{ t("pages.settings.agents.modelLabel") }}
                  <span class="dim">{{
                    t("pages.settings.agents.modelDim")
                  }}</span></span
                >
                <BaseSelect
                  v-model="agentForm.model"
                  :options="textModelSelectOptions"
                  :placeholder="t('pages.settings.agents.modelPlaceholder')"
                  searchable
                />
              </label>
              <div class="field-row">
              ...
              <label class="field">
                <span class="field-label">{{
                  t("pages.settings.agents.systemPromptLabel")
                }}</span>
                <textarea
                  ...
                  rows="12"
                  :placeholder="
                    t('pages.settings.agents.systemPromptPlaceholder')
                  "
                />
              </label>
              <div class="agent-card-foot">
                ...
                >
                  {{ t("pages.settings.agents.resetDefault") }}
                </button>
                <span
      ...
      <div v-else-if="tab === 'skills'" class="skills-layout">
        ...
        <aside class="skills-agent-list">
          <div class="skills-agent-title">
            {{ t("pages.settings.skills.agentsLabel") }}
          </div>
          <button
        ...
        <div class="settings-scroll skills-main">
          ...
          >
            ...
            <div class="empty-title">
              {{ t("pages.settings.skills.empty") }}
            </div>
            <div class="empty-desc">
              {{ t("pages.settings.skills.emptyDesc") }}
            </div>
          </div>
    ...
    <div v-if="cfgDialog" class="overlay" @click.self="cfgDialog = false">
      <form class="modal card config-modal" @submit.prevent="saveCfg">
        <div class="config-modal-head">
          <div>
            <div class="setup-kicker">
              {{
                cfgEditId
                  ? t("pages.settings.dialog.editConfig")
                  : t("pages.settings.dialog.newConfig")
              }}
            </div>
            <h2 class="modal-title">
              {{
                cfgEditId
                  ? t("pages.settings.dialog.editConfig")
                  : t("pages.settings.dialog.addConfig", {
                      label: t(
                        `pages.settings.aiServices.serviceTypes.${cfgForm.service_type}`,
                      ),
                    })
              }}
            </h2>
            <div class="modal-note">
              {{ t("pages.settings.dialog.modalNote") }}
            </div>
          </div>
          <span class="tag tag-accent">{{
            t(`pages.settings.aiServices.serviceTypes.${cfgForm.service_type}`)
          }}</span>
        </div>
        ...
        <label class="field">
          <span class="field-label">{{
            t("pages.settings.dialog.fieldConfigName")
          }}</span>
          <input
            v-model="cfgForm.name"
            class="input"
            :placeholder="t('pages.settings.dialog.fieldConfigNamePlaceholder')"
          />
        </label>
        <label class="field"
          ><span class="field-label">{{
            t("pages.settings.dialog.fieldProvider")
          }}</span>
          <BaseSelect
            v-model="cfgForm.provider"
            :options="providerSelectOptions"
            :placeholder="t('pages.settings.dialog.fieldProviderPlaceholder')"
            searchable
          />
        </label>
        <label class="field">
          <span class="field-label">{{
            t("pages.settings.dialog.fieldPriority")
          }}</span>
          <input
          ...
          <span class="field-hint">{{
            t("pages.settings.dialog.fieldPriorityHint")
          }}</span>
        </label>
        <label class="field"
        ...
        <div class="endpoint-hint">
          <span class="dim">{{
            t("pages.settings.dialog.actualEndpoint")
          }}</span>
          <span class="mono">{{ endpointHint }}</span>
        </div>
        <label class="field"
          ><span class="field-label">{{
            t("pages.settings.dialog.fieldModel")
          }}</span
          ><input
            v-model="cfgForm.modelStr"
            class="input"
            :placeholder="t('pages.settings.dialog.fieldModelPlaceholder')"
        /></label>
        <div
          v-if="cfgTestResult"
          class="test-result"
        ...
        <div class="modal-actions">
          ...
          <button type="button" class="btn" @click="cfgDialog = false">
            {{ t("pages.settings.dialog.cancel") }}
          </button>
          <button type="submit" class="btn btn-primary">
            {{ t("pages.settings.dialog.save") }}
          </button>
        </div>
      </form>
    </div>
    ...
    <div v-if="presetDialog" class="overlay" @click.self="presetDialog = false">
      <form class="modal card config-modal" @submit.prevent="applyHuobaoPreset">
        <div class="config-modal-head">
          <div>
            <div class="setup-kicker">
              {{ t("pages.settings.aiServices.presetDialogKicker") }}
            </div>
            <h2 class="modal-title">
              {{ t("pages.settings.aiServices.presetDialogTitle") }}
            </h2>
            <div class="modal-note">
              {{ t("pages.settings.aiServices.presetDialogNote") }}
            </div>
          </div>
          <span class="tag tag-success">{{
            t("pages.settings.aiServices.recommendedTag")
          }}</span>
        </div>
        <div class="huobao-grid">
          <label class="field">
            <span class="field-label"
              >Huobao API Key
              <span class="dim">{{
                t("pages.settings.aiServices.apiKeyHint")
              }}</span></span
            >
            <input
              ...
              class="input"
              type="password"
              :placeholder="t('pages.settings.aiServices.apiKeyPlaceholder')"
            />
            <span class="field-hint"
              >{{ t("pages.settings.aiServices.apiKeyRegisterPrompt")
              }}<a
                href="https://api.chatfire.site/"
                target="_blank"
                rel="noopener"
                >{{ t("pages.settings.aiServices.apiKeyRegisterLink") }}</a
              ></span
            >
          </label>
        </div>
        ...
        <div class="modal-actions">
          ...
          <button type="submit" class="btn btn-primary">
            {{ t("pages.settings.dialog.createAndEnable") }}
          </button>
        </div>
      </form>
    </div>
    ...
    >
      <form class="modal card" @submit.prevent="confirmAddSkill">
        <h2 class="modal-title">
          {{
            t("pages.settings.skills.addDialogTitle", {
        ...
        <label class="field">
          <span class="field-label"
            >{{ t("pages.settings.skills.skillDirLabel") }}
            <span class="dim">{{
              t("pages.settings.skills.skillDirDim")
            }}</span></span
          >
          <input
            v-model="newSkillForm.id"
            class="input"
            :placeholder="t('pages.settings.skills.skillDirPlaceholder')"
          />
        </label>
        <label class="field">
          <span class="field-label">{{
            t("pages.settings.skills.nameLabel")
          }}</span>
          <input
            v-model="newSkillForm.name"
            class="input"
            :placeholder="t('pages.settings.skills.namePlaceholder')"
          />
        </label>
        <label class="field">
          <span class="field-label">{{
            t("pages.settings.skills.descLabel")
          }}</span>
          <input
            v-model="newSkillForm.description"
            class="input"
            :placeholder="t('pages.settings.skills.descPlaceholder')"
          />
        </label>
        <div class="modal-actions">
...
const serviceMeta = computed(() => ({
  text: {
    label: t("pages.settings.aiServices.serviceTypes.text"),
    desc: t("pages.settings.aiServices.serviceDesc.text"),
  },
  image: {
    label: t("pages.settings.aiServices.serviceTypes.image"),
    desc: t("pages.settings.aiServices.serviceDesc.image"),
  },
  video: {
    label: t("pages.settings.aiServices.serviceTypes.video"),
    desc: t("pages.settings.aiServices.serviceDesc.video"),
  },
  audio: {
    label: t("pages.settings.aiServices.serviceTypes.audio"),
    desc: t("pages.settings.aiServices.serviceDesc.audio"),
  },
}));
const providerPresets = {
  text: {
    chatfire: {
      label: t("pages.settings.toast.providerPreset.chatfire"),
      baseUrl: "https://api.chatfire.site",
      models: ["gemini-3-pro-preview"],
    },
    openrouter: {
      label: t("pages.settings.toast.providerPreset.openrouter"),
      baseUrl: "https://openrouter.ai/api",
      models: ["google/gemini-3-flash-preview"],
    },
    openai: {
      label: t("pages.settings.toast.providerPreset.openai"),
  ...
  image: {
    ...
    gemini: {
      label: t("pages.settings.toast.providerPreset.gemini"),
      baseUrl: "https://api.chatfire.site",
      models: ["gemini-3-pro-image-preview"],
    },
    volcengine: {
      label: t("pages.settings.toast.providerPreset.volcengine"),
      baseUrl: "https://ark.cn-beijing.volces.com",
      models: ["doubao-seedream-4-0-250828"],
    },
    wan: {
      label: t("pages.settings.toast.providerPreset.wan"),
  ...
  video: {
    volcengine: {
      label: t("pages.settings.toast.providerPreset.huobaoVideo"),
      baseUrl: "https://api.chatfire.site/volcengine",
      models: ["doubao-seedance-1-5-pro-251215"],
    },
    vidu: {
      label: t("pages.settings.toast.providerPreset.vidu"),
      baseUrl: "https://api.vidu.com",
      models: ["viduq3-turbo"],
    },
    ali: {
      label: t("pages.settings.toast.providerPreset.ali"),
    ...
    wan: {
      label: t("pages.settings.toast.providerPreset.wan"),
      baseUrl: "http://localhost:8080",
      models: ["wan2.2-t2v-plus"],
    },
  },
  audio: {
    minimax: {
      label: t("pages.settings.toast.providerPreset.huobaoAudio"),
...
const huobaoPresetCards = [
  {
    serviceType: "text",
    label: t("pages.settings.toast.huobaoPresetCard.text"),
    provider: "chatfire",
    baseUrl: "https://api.chatfire.site",
  ...
  {
    serviceType: "image",
    label: t("pages.settings.toast.huobaoPresetCard.image"),
    provider: "gemini",
    baseUrl: "https://api.chatfire.site",
  ...
  {
    serviceType: "video",
    label: t("pages.settings.toast.huobaoPresetCard.video"),
    provider: "volcengine",
    baseUrl: "https://api.chatfire.site/volcengine",
  ...
  {
    serviceType: "audio",
    label: t("pages.settings.toast.huobaoPresetCard.audio"),
    provider: "minimax",
    baseUrl: "https://api.chatfire.site/minimax",
    model: "speech-2.8-hd",
    priority: 97,
...
async function delCfg(id) {
  ...
  toast.success(t("pages.settings.toast.deleted"));
  loadCfgs();
}
...
async function testCfgPayload(payload) {
  ...
  try {
    ...
    if (cfgTestResult.value.reachable)
      toast.success(t("pages.settings.toast.endpointReachable"));
...
async function saveCfg() {
  if (!cfgForm.provider) {
    toast.warning(t("pages.settings.toast.selectProvider"));
    return;
  }
...
const agentDefs = computed(() => [
  {
    ...
    label: t("pages.settings.agents.types.script_rewriter"),
    icon: "📝",
  },
  {
    type: "extractor",
    label: t("pages.settings.agents.types.extractor"),
    icon: "🔍",
  },
  {
    type: "storyboard_breaker",
    label: t("pages.settings.agents.types.storyboard_breaker"),
    icon: "🎬",
  },
  {
    type: "voice_assigner",
    label: t("pages.settings.agents.types.voice_assigner"),
    icon: "🎙",
  },
  {
    type: "grid_prompt_generator",
    label: t("pages.settings.agents.types.grid_prompt_generator"),
    icon: "🖼",
  },
]);
...
const defaultPrompts = computed(() =>
  locale.value === "en" ? defaultPromptsEn : defaultPromptsZh,
);
...
function toggleAgentEdit(type) {
  ...
  agentForm.system_prompt =
    cfg?.system_prompt || defaultPrompts.value[type] || "";
...
async function saveAgentCfg(type) {
  ...
  try {
    ...
    toast.success(
      t("pages.settings.toast.agentSaved", {
        label: agentDefs.value.find((a) => a.type === type)?.label || type,
      }),
    );
    setTimeout(() => {
